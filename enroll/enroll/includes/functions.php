...
function checkAttempts($enroll_key)
{
    ...
    try {
        ...
        $sql = "SELECT Attempts as attempts, lastlogin FROM ".$tbl_attempts." WHERE IP = :ip and enroll_key = :enroll_key";
...
function mySqlErrors($response)
{
    ...
    switch (substr($response, 0, 22)) {
        ...
        case 'Error: SQLSTATE[23000]':
            echo "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>Enroll key already exists</div>";
            break;
